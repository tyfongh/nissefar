...
uint32_t decode_utf8_at(const std::string &text, size_t &index) {
  if (index >= text.size()) {
    return 0;
  }
  ...
  if ((c0 & 0xF8) == 0xF0) {
    ...
    const uint32_t cp = ((c0 & 0x07) << 18) | ((c1 & 0x3F) << 12) |
                        ((c2 & 0x3F) << 6) | (c3 & 0x3F);
    index += 4;
...
int codepoint_display_width(uint32_t cp) {
  ...
  static constexpr Range combining[] = {
      ...
      {0x17DD, 0x17DD}, {0x180B, 0x180D}, {0x1885, 0x1886},
  ...
  if (in_ranges(cp, combining, std::size(combining))) {
    return 0;
  }

  static constexpr Range wide[] = {
      ...
      {0x3300, 0x4DBF}, {0x4E00, 0xA48C}, {0xA490, 0xA4C6},
      ...
      {0x1F6F4, 0x1F6F9}, {0x1F910, 0x1F93E}, {0x1F940, 0x1F94C},
      {0x1F950, 0x1F96B}, {0x1F980, 0x1F997}, {0x1F9C0, 0x1F9C0},
      {0x1F9D0, 0x1F9E6}, {0x20000, 0x2FFFD}, {0x30000, 0x3FFFD}};

  if (in_ranges(cp, wide, std::size(wide))) {
    return 2;
  }
...
size_t utf8_display_width(const std::string &text) {
  ...
  size_t i = 0;
  while (i < text.size()) {
    width += static_cast<size_t>(codepoint_display_width(decode_utf8_at(text, i)));
  }
  return width;
}
...
std::string utf8_truncate_to_width(const std::string &text, size_t max_width) {
  ...
  size_t i = 0;
  while (i < text.size()) {
    const size_t codepoint_start = i;
    const uint32_t cp = decode_utf8_at(text, i);
    const size_t cp_width = static_cast<size_t>(codepoint_display_width(cp));
    if (width + cp_width > max_width) {
      break;
    }
    out.append(text, codepoint_start, i - codepoint_start);
    width += cp_width;
  }
...
void pad_right(std::string &text, size_t num, const std::string &pad_char) {
  const size_t width = utf8_display_width(text);
  const size_t len = width >= num ? 0 : (num - width);
